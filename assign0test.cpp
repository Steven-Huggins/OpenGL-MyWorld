...
void drawScene() {
    ...
    glBindTexture(GL_TEXTURE_2D, floorTexture);
    glBegin(GL_QUADS);
    glTexCoord2f(0.0, 0.0); glVertex3f(-3.0f, 0.1f, 4.1f);
    glTexCoord2f(1.0, 0.0); glVertex3f(1.8f, 0.1f, 12.0f);
    glTexCoord2f(1.0, 1.0); glVertex3f(36.2f, 0.1f, -23.0f);
    glTexCoord2f(0.0, 1.0); glVertex3f(28.0f, 0.1f, -30.0f);
    glEnd();
...
void initTower() {
    init_texture(Tower_textures, 2);

    char filename1[] = "Tower.png";
    set_texture(Tower_textures, 0, filename1);

    char filename2[] = "grey_texture.bmp";
    set_texture(Tower_textures, 1, filename2);
}
...
void drawTower() {
    glPushMatrix();
    glTranslatef(-5.0f, 2.0f, 1.0f); //调整位置
    glScalef(10.0f, 15.0f, 10.0f); // 调整比例
    Tower();
    glPopMatrix();
}
...
void update(int value) {
    ...
    float radius = 1.0f;
    float theta = glutGet(GLUT_ELAPSED_TIME) / 1000.0f * 20.0f * 3.1415926f;
...
void drawBurger() {
    ...
    glScalef(10.0f, 10.0f, 10.0f); // 调整缩放比例
    burger();
    glPopMatrix();
}
void display() {
